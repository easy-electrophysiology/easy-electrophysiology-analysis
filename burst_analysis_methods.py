...
def detect_burst_start_stop_index(peak_times_ms, burst_cfgs):
    """
    Entry function for burst detection. Returns the burst start / stop indexes of the peak_times_ms array. Uses the logISI
    or interval method
    ...
    if burst_cfgs["detection_method"] == "log_isi":
        ...
        if burst_cfgs["max_long_isi_ms"] > burst_cfgs["max_short_isi_ms"]:
            ...
            burst_start_idx, burst_stop_idx = log_isi_burst_detection_long(peak_times_ms,
                                                                           isi_ms,
            ...
            burst_start_idx, burst_stop_idx, __ = log_isi_burst_detection_short(isi_ms,
                                                                                burst_cfgs["min_spikes_per_burst"],
                                                                                burst_cfgs["max_long_isi_ms"])
...
def log_isi_burst_detection_long(peak_times, isi, min_spikes_per_burst, max_short_isi, max_long_isi):
    """
    Detect bursts using the logISI method as described in:

    Pasquale V, Martinoia S, Chiappalone M. A self-adapting approach for the detection of bursts and network bursts in neuronal cultures.
    ...
    sort_idx = np.lexsort((all_edge_sort[1, :], all_edge_sort[0, :]))  # first sort by the times, then when for times that are identical sort by row idx 1 (i.e. keys above)
    ...
    burst_train = []
    for outer_begin_idx in all_outer_burst_begin_idx:  # now we are indexing all_edge_sort (which itself contains peak times / indexes)
        ...
        # for each burst, look at the sub-bursts and split if more than one sub-burst found.
        ...
        if all_sub_burst_start_idx.size > 1:
            ...
            for i in range(sub_burst_begin.size):
                ...
                if i == sub_burst_begin.size - 1:
                    burst_start_end = [all_edge_sort[2, sub_burst_begin[i]],
                                       all_edge_sort[2, sub_burst_end[i]]]
                else:
                    burst_start_end = [all_edge_sort[2, sub_burst_begin[i]],    # for sub bursts, finish each sub-burst 1 spike before the start of the next sub-burst.
                                       all_edge_sort[2, sub_burst_end[i]] - 1]  # This has to do be done here due to the nature of indexing AllEdgeSort

                burst_train.append(burst_start_end)

        else:
            burst_start_end = [all_edge_sort[2, outer_begin_idx],
...
def log_isi_burst_detection_short(isi, min_spikes_per_burst, max_isi):
    """
    Find the idx of rising (burst start) and falling (burst finish) edges for
    log ISI burst detection.
    """
...
def find_burst_edges(isi, max_short_isi):
    """
...
def calculate_log_threshold(isi, burst_cfgs):
    """
    Calculate the threshold ISI which separates intra-burst ISI.

    In a timeseries with bursting, the inter-spike interval (ISI) can be separated into shorter
    intra-burst ISI and the ISI expected under normal conditions (e.g. Poisson distributed spikes). Taking
    the histogram of the logarithm of the ISI (note ISI for poisson distributed spikes is exponential)
    gives peaks at the intra-burst interval, and the expected larger peak. Using the algorithm described in
    Pasquale et al (2009) these peaks can be optimally separated and the ISI which best separates them is used
    as the thershold for burst detection.

    INPUTS:
    isi: 1 x N array of inter-spike (or inter-event for events analysis) intervals.

    burst_cfgs: dictionary containing options -
                                               min_void_parameter - minimum void parameter accepted for distiguishing two peaks
                                               ...
                                               lowess_fraction - fraction of total number of samples used for the local regression
                                               intraburst_peak_cutoff_ms - threshold used to determine region of histogram that contains intraburst peak
                                               min_samples_between_peaks  - min samples between peaks used for peak detection
    OUTPUTS:

        max_long_isi - max ISI threshold used as maxISI2, or ISIth (see Pasqule et al., 2009).
    ...
    NOTE:
        The variable info_for_plotting must be updated after each function as if an error
        occurs the plot is update with parameters calculated thus far.

    Cotterril (2016). A comparison of computational methods for detecting bursts in neuronal spike trains and their
    application to human stem cell-derived neuronal networks. J Neurophysiol. 116(2), 306-321.
    """
    info_for_plotting = {"bins": None,
                         "count_prob": None,
                         "min_minimum_index": None,
                         "intra_burst_peak_idx": None,
                         "larger_isi_peak_indexes": None,
                          "void_parameters": None}

    bins, count_prob = calculate_logisi_histogram(isi,
                                                  burst_cfgs)
    ...
    intra_burst_peak_idx, \
        larger_isi_peak_indexes = get_log_isi_hist_peaks(burst_cfgs, bins, count_prob)
    ...
    minimum_indexes, info_for_plotting["void_parameters"] = get_minimums_with_valid_void_parameter(count_prob,
                                                                                                   intra_burst_peak_idx,
    ...
    max_long_isi_log = bins[min_minimum_index]  # take the lowest idx void param > min void param
    max_long_isi = 10**max_long_isi_log

    info_for_plotting["min_minimum_index"] = min_minimum_index

    return max_long_isi, info_for_plotting


def calculate_logisi_histogram(isi, burst_cfgs):
    """
    Calculate the histogram of log10 ISI with centered bins. Histogram frequencies are
    converted to probabilities. Optional smoothing of the histogram with LOWESS smoothing.
    ...
    if burst_cfgs["histogram_smoothing"]:
        ...
        frac = lowess_frac if len(count_prob) * lowess_frac > 5 else 5 / len(count_prob)  # default to span = 5 points similar to matlab smooth function
        count_prob = statsmodels.nonparametric.lowess(exog=format_bins, endog=count_prob, return_sorted=False, frac=frac)

    return format_bins, count_prob

def get_log_isi_hist_peaks(burst_cfgs, bins, count_prob):
    """
    Find intra-burst peak and other peaks.
    ...
    other_peak_indexes = scipy.signal.find_peaks(count_prob,
                                                 distance=burst_cfgs["min_samples_between_peaks"])[0]
...
def get_minimums_with_valid_void_parameter(g, intra_burst_peak_idx, larger_isi_peak_indexes, min_void_parameter):
    """
    g is the distribution g(x) of x = log(ISI) (i.e. the count_prob from calculate_logisi_histogram(), g(x) chosen to match paper).

    The void parameter is the ratio of the height of the minimum between two peaks to the geometric average of the two peaks. If the
    minimum is zero, the peaks are perfectly separated (void = 0). If void = 0.5, the minimum between the two peaks is half the
    geometric average of the height of the two peaks.

    The void parameter is calculated between the intra-burst peak and every other peak in the histogram.
...
def calculate_burst_parameters(peak_times, burst_start_idx, burst_stop_idx):
    ...
    all_intra_burst_ISI = [np.mean(np.diff(bust_peak_times * 1000)) for bust_peak_times in all_burst_peak_times]  # burst peak times must be in s later, so this awkward conversion needed

    return burst_lengths_ms, num_spikes_in_burst, inter_burst_intervals, rec_fraction_of_spikes_in_burst, all_burst_idx, all_burst_peak_times, all_intra_burst_ISI
