...
from ephys_data_methods import heka_loader
...
class ImportData:
    ...
    """
    def __init__(self, full_filepath, ext, cfgs, mw):
        ...
        self.channels, \
            self.reader, \
            self.neo_block = self.load_file_with_neo()
    ...
    def load_data(self):
        ...
        channel_1, channel_1_idx, \
            channel_2, channel_2_idx = self.get_channel_data_info()

        if channel_1 is None or self.channel_is_proxy(channel_1_idx, channel_1=True):
            return

        # Get channel type (Im or Vm), units and load data structure
        channel_1_units, channel_1_type = self.get_channel_type(channel_1,
                                                                channel_1_idx)
        if not channel_1_type:
            return

        if channel_2 and not self.channel_is_proxy(channel_2_idx):

            channel_2_units, channel_2_type = self.get_channel_type(channel_2,
                                                                    channel_2_idx)

            if not self.channel_2_exists_and_is_different_type_to_channel_1(channel_1_type,
                                                                            channel_2_type):
                return

            Data = RawData(self.neo_block, 2, self.cfgs,
                           channel_1_type, channel_1_units, channel_1_idx,
                           channel_2_type, channel_2_units, channel_2_idx,
                           self.reader, self.ext)
        else:
            Data = RawData(self.neo_block, 1, self.cfgs,
                           channel_1_type, channel_1_units, channel_1_idx,
                           None, None, None,
                           self.reader, self.ext)
        return Data
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def load_file_with_neo(self):
        ...
        if self.ext == ".H5":  # TODO: own function
            neo_block = reader.read_block()

        elif self.ext == ".DAT":  # TODO: CHANGE NAME
            heka = heka_loader.OpenHeka(self.mw, self.full_filepath)  # this will exec to get user selected Tree index
...
# Processing Channels --------------------------------------------------------------------------------------------------------------------------------
    ...
    def get_channel_data_info(self):
        """
    ...
    def get_users_default_channels(self):
        """
    ...
    def get_default_channels(self):
        ...
        if num_channels > 2:
            channel_1_idx, channel_2_idx, channel_1, channel_2 = self.get_channels_from_user_input()
    ...
    def channel_is_proxy(self, channel_idx, channel_1=False):
        """
        ...
        than the number of channels neo can find. In this instance neo will fill with a proxy analogsignals
        but this will crash data loading class. Here check if any are proxy (absence of data i.e. 'magnitude attribute)
        and inform the user that this channel does not exist.

        Show error message if channel 1 index is tested (if another channel fails, just channel 1 will be loaded)
        """
        ...
        if channel_1 and channel_is_proxy:
            self.show_warning("Load File Error",
    ...
    def channel_2_exists_and_is_different_type_to_channel_1(self, channel_1_type, channel_2_type):
    ...
    def get_channel_type(self, channel, channel_idx):
        """
        ...
        channel_units, \
            channel_type = self.get_channel_type_from_channel_units(channel_units,
                                                                    channel_idx)

        if channel_units == "not_recognised":
            channel_units, channel_type = self.get_user_input_channel_units(channel[4].strip(),
                                                                            channel_idx)

        return channel_units, channel_type

    def get_channel_type_from_channel_units(self, channel_units, channel_idx):
        """
        """
        if channel_units == "mV":
            channel_type = "Vm"
            ...
            channel_units, channel_type = self.process_non_default_channel_units(channel_units,
                                                                                 channel_idx)
    ...
    def get_user_input_channel_units(self, channel_units, channel_idx):
        ...
        while True:
            ...
            channel_units, channel_type = self.get_channel_type_from_channel_units(channel_units,
                                                                                   channel_idx)
    ...
    def process_non_default_channel_units(self, channel_units, channel_idx):
        """
        ...
        is not specified, prompt the user to set defaults, with an additional check that the header makes sense (in some cases e.g. Axograph files
        the header is in A but data is scaled to nA; see show_bad_header_units_warning())

        Otherwise, load the file as per the defaults. HEKA files are automatically converted from A / V.
        ...
        if self.ext == ".DAT":
            ...
            channel_units = "pA" if self.cfgs.file_load_options["default_im_units"]["assume_pa"] else self.cfgs.file_load_options["default_im_units"]["pa_unit_to_convert"]

        elif channel_type == "Vm" and self.cfgs.file_load_options["default_vm_units"]["on"]:
            channel_units = "mV" if self.cfgs.file_load_options["default_vm_units"]["assume_mv"] else self.cfgs.file_load_options["default_vm_units"]["mv_unit_to_convert"]
    ...
    def get_channels_from_user_input(self):
        """
        """
        while True:
...
# Handle non-pA or mV channel types ------------------------------------------------------------------------------------------------------------------
    ...
    def show_bad_header_units_warning(self, channel_type, channel_units, channel_idx):
        """
        ...
        guessed_units = self.check_if_header_units_are_correct(channel_idx,
                                                               channel_type)
        ...
        self.mw.show_messagebox("Channel Units Error",
                                message_string)

    def check_if_header_units_are_correct(self, channel_idx, im_or_vm):
        """
    ...
    def show_warning(self, title, text):
        ...
        """
        ShowChannelsWarning(self.mw, self.cfgs, self.channels, title, text)
...
class RawData:
    ...
    def __init__(self, neo_block, num_chans, cfgs,
        ...
        self.im_array = np.zeros((self.num_recs,
                                  self.num_samples))  # empty must be zero, if empty or nan cannot be properly plot by pyqtgraph in the one-channel case
    ...
    def handle_generate_axon_protocol(self, reader, cfgs):
        ...
        the Im protocol will be generated and loaded to the second channel, handling any errors.
        """
