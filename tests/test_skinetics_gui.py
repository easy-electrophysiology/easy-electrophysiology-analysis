...
class TestsKinetics:
    ...
    def get_loaded_file(tgui, main_key, param_key, rec):
        return tgui.get_skinetics_param_from_skinetics_data(
            tgui.mw.loaded_file.skinetics_data, main_key, param_key, rec
        )
    ...
    def get_stored_tabledata(tgui, main_key, param_key, rec, filenum=0):
        return np.array(
            [
                dict_[param_key]
                for dict_ in utils.flatten_dict(tgui.mw.stored_tabledata.skinetics_data[filenum], rec, main_key)
            ]
        )
    ...
    def get_spiketimes_within_bounds(tgui, rec_from, rec_to, bounds_vm):
        """ """
        data = utils.np_empty_nan((tgui.adata.num_recs, tgui.adata.max_num_spikes))
    ...
    def set_ahp(tgui, fahp_or_mahp, to, from_):
        if fahp_or_mahp == "fahp":
            tgui.enter_number_into_spinbox(tgui.mw.dialogs["skinetics_options"].dia.fahp_start, to)
            tgui.enter_number_into_spinbox(tgui.mw.dialogs["skinetics_options"].dia.fahp_stop, from_)
        elif fahp_or_mahp == "mahp":
            tgui.enter_number_into_spinbox(tgui.mw.dialogs["skinetics_options"].dia.mahp_start, to)
            tgui.enter_number_into_spinbox(tgui.mw.dialogs["skinetics_options"].dia.mahp_stop, from_)

    @staticmethod
    def data_within_bounds(tgui, data, rec, bounds_vm):
        if bounds_vm:
            within_bounds_bool = tgui.adata.get_within_bounds_spiketimes(
                rec, bounds_vm, tgui.time_type, return_as_bool=True, cut_nan=True
            )
    ...
    def check_spiketimes(self, tgui, rec_from, rec_to, bounds_vm, filenum):
        ...
        loaded_file_spike_times = test_utils.get_spike_times_from_spike_info(
            tgui.adata, tgui.mw.loaded_file.skinetics_data
        )
        tabledata_spike_times = test_utils.get_spike_times_from_spike_info(
            tgui.adata, tgui.mw.stored_tabledata.skinetics_data[filenum]
        )

        tgui.check_spiketimes(tgui, loaded_file_spike_times, test_spike_times, "model spiketimes")
        tgui.check_spiketimes(tgui, tabledata_spike_times, test_spike_times, "tabledata_spiketimes")
        ...
        for rec in range(rec_from, rec_to + 1):
            ...
            self.check_plot(tgui, tgui.clean(test_spike_times[rec]), rec, "peak_plot", "xData")
    ...
    def check_skinetics_threshold(self, tgui, rec, analyse_specific_recs, bounds_vm, filenum):
        ...
        """
        test_spike_sample_idx, rec_from, rec_to = tgui.handle_analyse_specific_recs(
            analyse_specific_recs, tgui.adata.spike_sample_idx
        )

        test_thr_vm = tgui.adata.resting_vm
        test_thr_time = tgui.adata.time_array[rec][tgui.clean(test_spike_sample_idx[rec]).astype(int)]

        test_thr_time = self.data_within_bounds(tgui, test_thr_time, rec, bounds_vm)
        ...
        self.check_plot(tgui, np.tile(test_thr_vm, len(test_thr_time)), rec, "thr_plot", "yData")
        self.check_plot(tgui, test_thr_time, rec, "thr_plot", "xData")
    ...
    def check_skinetics_peak_and_amplitude(self, tgui, rec, bounds_vm, filenum):
        ...
        self.check_plot(tgui, test_peaks, rec, "peak_plot", "yData")
    ...
    def check_skinetics_half_widths(self, tgui, rec, bounds_vm, filenum):
        ...
        plot_hw_data = self.check_plot(
            tgui,
            False,
            rec,
            "fwhm_plot",
            "xData",
            sub_data_dict=dict(type="half_rise_half_decay", data=len(test_half_widths[rec])),
        )
    ...
    def check_skinetics_ahps(self, tgui, rec, bounds_vm, filenum):
        ...
        self.check_plot(
            ...
            "mahp_plot",
            "yData",
            sub_data_dict=dict(type="subtract", data=tgui.adata.resting_vm),
        )
        self.check_plot(
            tgui,
            test_actual_ahp,
    ...
    def test_skinetics_main_results(self, tgui, spike_detection_method, analyse_specific_recs, run_with_bounds):
        ...
        for filenum in range(3):
            bounds_vm = tgui.run_artificial_skinetics_analysis(
                run_with_bounds=run_with_bounds,
                spike_detection_method=spike_detection_method,
                override_mahp_fahp_defaults=True,
            )
    ...
    def test_skinetics_rise_times(self, tgui, analyse_specific_recs, min_max_cutoff):
        ...
        for rec in range(rec_from, rec_to + 1):
            ...
            plot_rise_data = self.check_plot(
                tgui,
                False,
                rec,
                "rise_plot",
    ...
    def test_skinetics_decay_times(self, tgui, analyse_specific_recs, min_bound, min_max_cutoff):
        ...
        for rec in range(rec_from, rec_to + 1):
            ...
            plot_decay_data = self.check_plot(
                tgui,
                False,
                rec,
                "decay_plot",
    ...
    def test_max_slope(self, tgui, analyse_specific_recs, n_samples_rise_decay):
        ...
        (
            ...
            rec_rise_y_datas,
            rec_decay_y_datas,
        ) = tgui.adata.get_max_slope(n_samples_rise=n_samples_rise, n_samples_decay=n_samples_decay)
        for rec in range(rec_from, rec_to + 1):
    ...
    def test_manual_selection_and_deletion(self, tgui):
        ...
        for filenum in range(3):
            ...
            for spikes_to_select in get_settings(tgui.speed, tgui.analysis_type)["manually_sel"]:
                ...
                spike_params = self.check_all_skinetics_on_a_single_spike(
                    tgui, rec, rec_spike_idx, skinetics_keys, filenum
                )
            ...
            for spikes_deleted, spikes_to_delete in enumerate(
                get_settings(tgui.speed, tgui.analysis_type)["manually_del"]
            ):
                rec, rec_spike_idx, list_spike_idx = spikes_to_delete
                tgui.mw.update_displayed_rec(rec)

                tgui.click_upperplot_spotitem(
                    tgui.mw.loaded_file_plot.skinetics_plot_dict["peak_plot"],
                    rec_spike_idx,
                    doubleclick_to_delete=True,
                )

                for key in skinetics_keys:
                    inserted_spike_data[key].pop(
                        list_spike_idx - spikes_deleted
                    )  # iterate from early idx to higher, so delete higher idx each time one is popped
    ...
    def test_fahp_and_ahp_timings(self, tgui, analyse_specific_recs, fahp_start_len_ms, mahp_start_len_ms):
        ...
        tgui.set_skinetics_ahp_spinboxes(
            tgui.mw.dialogs["skinetics_options"].dia,
            fahp_start=fahp_start_ms,
            fahp_stop=fahp_start_ms + fahp_len_ms,
            mahp_start=mahp_start_ms,
            mahp_stop=mahp_start_ms + mahp_len_ms,
        )
        ...
        for rec in range(rec_from, rec_to + 1):
            ...
            test_fahp = self.index_out_ahp_regions(
                tgui, spike_peaks_idx, rec, fahp_start_ms, fahp_len_ms, tgui.adata.fs
            )
            test_mahp = self.index_out_ahp_regions(
                tgui, spike_peaks_idx, rec, mahp_start_ms, mahp_len_ms, tgui.adata.fs
            )
    ...
    def test_phase_plot_analysis(self, tgui, run_with_bounds, analyse_specific_recs, interp):
        ...
        for filenum in range(3):
            ...
            (
                dialog,
                window_samples,
            ) = self.setup_phase_space_dialog_and_set_windows_samples(tgui)
            ...
            for rec_idx in range(rec_from, rec_to + 1):
                for ap_idx, peak_time in enumerate(test_spike_times[rec_idx]):
                    ...
                    res = self.check_and_return_all_spike_and_phase_plot_data(
                        tgui,
                        dialog,
                        rec_idx,
                        ap_idx,
    ...
    def check_and_return_all_spike_and_phase_plot_data(
        self,
        tgui,
        dialog,
        rec_idx,
        ...
        spike_x = (
            file_data.time_array[rec_idx][peak_idx - window_samples : peak_idx + window_samples + 1]
            - file_data.time_array[rec_idx][peak_idx - window_samples]
        )
        ...
        (
            ...
            test_dvm_max_vm,
            test_dvm_max_vmdiff,
        ) = self.check_phase_plot_vm_vm_diff_and_all_params(tgui, dialog, spike_y, rec_idx, ap_idx, interp, use_adata)
        ...
        res = SimpleNamespace(
            ...
            spike_y=spike_y,
            test_vm=test_vm,
            test_vm_diff=test_vm_diff,
            test_threshold_vm=test_threshold_vm,
            test_threshold_vdiff=test_threshold_vdiff,
            test_vm_max_vm=test_vm_max_vm,
            test_vm_max_vmdiff=test_vm_max_vmdiff,
            test_dvm_min_vm=test_dvm_min_vm,
            test_dvm_min_vmdiff=test_dvm_min_vmdiff,
            test_dvm_max_vm=test_dvm_max_vm,
            test_dvm_max_vmdiff=test_dvm_max_vmdiff,
        )
        return res
    ...
    def check_copied_data(self, tgui, dialog, rec_idx, ap_idx, res):
        ...
        test_headers = (
            first_headers
            + ["Spike Timepoint (s) " + str(idx) for idx in range(len(res.spike_x))]
            + ["Spike Vm (mV) " + str(idx) for idx in range(len(res.spike_y))]
            + ["Phase Vm (mV) " + str(idx) for idx in range(len(res.test_vm))]
            + ["Phase dVm (mV/ms) " + str(idx) for idx in range(len(res.test_vm_diff))]
            + last_headers
        )
    ...
    def check_copy_phase_plot_params(self, dialog, rec_idx, ap_idx, res, plot_copy_headers):
        ...
        assert utils.allclose(
            copied_data.iloc[:, 1],
            self.res_to_spike_info(rec_idx, ap_idx, res, parameters_only=True),
        )

    def check_phase_plot_vm_vm_diff_and_all_params(
        self, tgui, dialog, spike_y, rec_idx, ap_idx, interp, use_adata=True
    ):
        """
        ...
        if interp:
            test_vm_diff = core_analysis_methods.interpolate_data(
                test_vm_diff, np.arange(len(test_vm)), "cubic", 100, 0
            )
        ...
        (
            ...
            test_dvm_max_vm,
            test_dvm_max_vmdiff,
        ) = self.get_and_test_phase_plot_parameters(
            tgui, rec_idx, ap_idx, dialog, test_vm, test_vm_diff, interp, use_adata
        )
    ...
    def get_and_test_phase_plot_parameters(
        self,
        tgui,
        rec_idx,
        ap_idx,
    ...
    def peak_time_to_idx(self, tgui, rec_idx, peak_time):
        ...
        peak_idx = core_analysis_methods.quick_get_time_in_samples(tgui.mw.loaded_file.data.ts, peak_time)
        return peak_idx
        ...
        if parameters_only:
            return np.hstack(
                [
                    ...
                    ap_idx + 1,
                    res.spike_time,
                    res.test_threshold_vm,
                    res.test_threshold_vdiff,
                    res.test_vm_max_vm,
                    res.test_vm_max_vmdiff,
                    res.test_dvm_max_vm,
                    res.test_dvm_max_vmdiff,
                    res.test_dvm_min_vm,
                    res.test_dvm_min_vmdiff,
                ]
            )
        else:
            return np.hstack(
    ...
    def set_and_get_phase_analysis_window_samples(self, tgui, dialog, window_ms):
        """"""
        tgui.enter_number_into_spinbox(dialog.dia.window_size_spinbox_left, window_ms)
        tgui.enter_number_into_spinbox(dialog.dia.window_size_spinbox_right, window_ms)
        window_samples = round(window_ms / (tgui.mw.loaded_file.data.ts * 1000))

        return window_samples

    def setup_for_phase_plot_tests(
        self,
        tgui,
        run_with_bounds=False,
        spike_detection_method="auto",
        override_mahp_fahp_defaults=True,
        first_deriv_cutoff=10,
        thr_search_region=2,
        window_ms=2,
        ...
        tgui.run_artificial_skinetics_analysis(
            run_with_bounds=run_with_bounds,
            spike_detection_method=spike_detection_method,
            override_mahp_fahp_defaults=override_mahp_fahp_defaults,
            first_deriv_cutoff=first_deriv_cutoff,
        ...
        for filenum in range(3):
            ...
            for __ in range(20):
                (
                    ...
                    ap_idx,
                    peak_time,
                ) = self.get_random_rec_and_ap_idx_and_set_on_phase_dialog(tgui, dialog)
    ...
    def test_manual_deleting_spikes(self, tgui):
        ...
        peak_time = self.quick_check_displayed_spike(
            tgui, dialog, rec_idx - 1, 0, window_samples
        )  # auto goes to first spike
        ...
        peak_time = self.quick_check_displayed_spike(
            tgui, dialog, rec_idx, 0, window_samples
        )  # auto goes to first spike
        assert peak_time == rec_4_times[-1]
        tgui.click_upperplot_spotitem(
            tgui.mw.loaded_file_plot.skinetics_plot_dict["peak_plot"],
    ...
    def test_manual_deleting_all_spikes(self, tgui):
        ...
        for rec, rec_data in enumerate(tgui.mw.loaded_file.skinetics_data):
            ...
            for ap_idx in range(num_spikes):
                if (
                    rec == len(tgui.mw.loaded_file.skinetics_data) - 1 and ap_idx == num_spikes - 1
                ):  # stop before the very last spike
                    break
                tgui.click_upperplot_spotitem(
                    tgui.mw.loaded_file_plot.skinetics_plot_dict["peak_plot"],
                    0,
                ...
                if ap_idx != num_spikes - 1:
                    self.quick_check_displayed_spike(
                        tgui, dialog, rec, 0, window_samples
                    )  # cant test when all spieks in rec deleted
    ...
    def test_manually_selecting_spikes(self, tgui):
        ...
        rec_4_times = np.sort(list(tgui.mw.loaded_file.skinetics_data[rec_idx].keys())).astype(np.float64)
        del_spike_time = self.quick_check_displayed_spike(
            tgui, dialog, rec_idx, ap_idx, window_samples
        )  # cant test when all spieks in rec deleted
    ...
    def check_last_spike_messagebox(self, tgui):
        ...
        assert (
            ...
            == "<p align='center'>There are no analysed action potentials. Phase-plot window will now close.</p>"
        )
    ...
    def quick_check_displayed_spike(self, tgui, dialog, rec_idx, ap_idx, window_samples):
        ...
        peak_idx = self.peak_time_to_idx(tgui, rec_idx, peak_time)
        self.check_and_return_all_spike_and_phase_plot_data(
            tgui,
            dialog,
            rec_idx,
            ap_idx,
