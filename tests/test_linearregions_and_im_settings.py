...
class TestLinearRegionsImSettings:
    ...
    def check_linearregion_bounds_right(bounds_obj, region_type, x_axis):
        ...
        """
        tmp_bounds = bounds_obj.bounds[
            region_type
        ].getRegion()  # save default boundary position so boundaries are left in default after test finished
        ...
        result = bounds_obj.bounds[region_type].getRegion() == (
            x_axis[1] - min_difference - offset,
            x_axis[1],
        )
    ...
    def check_linearregion_bounds_left(bounds_obj, region_type, x_axis):
        ...
        result = bounds_obj.bounds[region_type].getRegion() == (
            x_axis[0],
            x_axis[0] + min_difference + offset,
        )
    ...
    def check_lr_baseline(tgui, bounds_obj, region_type, vm_or_im_array, analysis_cfg):
        ...
        bounds_obj.bounds[region_type].setRegion(
            (tgui.test_bl_lr_start_time, tgui.test_bl_lr_stop_time)
        )  # make the assumption any tested rec will have time period at least 50 ms long
        # get bounds idx
        [start_sample, stop_sample] = tgui.get_test_idx_from_time_bounds(
            analysis_cfg, region_type + "_lowerbound", region_type + "_upperbound", rec
        )
        ...
        test_average = np.mean(vm_or_im_array[rec][tgui.test_bl_lr_start_idx : tgui.test_bl_lr_stop_idx + 1])
    ...
    def check_im_protocol(self, tgui, analysis, correctly_filled):
        ...
        if correctly_filled:
            self.check_im_protocol_widgets_no_error(tgui, set_im_button, run_analysis_button)
            loaded_file_analysis_df = tgui.get_analysis_dataframe(analysis)
            result = ~np.all(np.isnan(loaded_file_analysis_df["record_num"]))
        else:
            self.check_im_protocol_widgets_error(tgui, set_im_button, run_analysis_button)
            loaded_file_analysis_df = tgui.get_analysis_dataframe(analysis)
            result = np.all(np.isnan(loaded_file_analysis_df["record_num"]))
        return result
    ...
    def check_user_im_input(self, tgui, num_recs, analysis, correctly_filled, within_recs=False):
        ...
        tgui.mw.loaded_file.init_analysis_results_tables()
        self.check_user_im_input_widgets(
            tgui,
            num_recs,
            set_im_button,
            run_analysis_button,
    ...
    def check_im_protocol_widgets_error(tgui, analysis_set_im_button, analysis_calc_button):
        ...
        QtCore.QTimer.singleShot(
            500,
            lambda: tgui.mw.dialogs["im_inj_protocol"].dia.im_injprot_start_spinbox.setValue(0.1),
        )
        QtCore.QTimer.singleShot(
            1000,
            lambda: tgui.left_mouse_click(
                tgui.mw.dialogs["im_inj_protocol"].dia.im_injprot_buttonbox.button(QtWidgets.QDialogButtonBox.Cancel)
            ),
        )
    ...
    def check_user_im_input_widgets(
        self,
        tgui,
        num_recs,
        analysis_set_im_button,
        analysis_calc_button,
        correctly_filled,
        within_recs,
    ):
        """
    ...
    def check_messagebox_num_recs_error(self, messagebox):
        assert (
            "Length of input Im protocol must be the same as number of records that will be analyzed"
            in messagebox.text()
        )
        messagebox.close()
    ...
    def test_spikecount_bounds_none(self, tgui):
        tgui.switch_to_spikecounts_and_set_im_combobox(spike_bounds_on=False, im_groupbox_on=False)
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_exp_lr"] not in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount no bounds - upper_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_bl_lr"] not in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount no bounds - upper_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_exp_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount no bounds - lower_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_bl_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount no bounds - lower_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.axlines["lower_bl_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount no bounds - axlines lower_bl_lr"

    def test_spikecount_bounds_upper(self, tgui):
        tgui.switch_to_spikecounts_and_set_im_combobox(spike_bounds_on=True, im_groupbox_on=False)
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_exp_lr"] in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount bounds upper - upper_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_bl_lr"] not in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount bounds upper - upper_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_bl_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount bounds upper - lower_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_exp_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount bounds upper - lower_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.axlines["lower_bl_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount bounds upper - axline lower_bl_lr"

    def test_spikecount_bounds_all(self, tgui):
        tgui.switch_to_spikecounts_and_set_im_combobox(spike_bounds_on=True, im_groupbox_on=True)
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_exp_lr"] in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount bounds all - upper_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_bl_lr"] not in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount bounds all - upper_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_bl_lr"] in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount bounds all - lower_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_exp_lr"] in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount bounds all - lower_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.axlines["lower_bl_lr"] in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount im protocol - lower bl axline"

    def test_spikecount_bounds_im_protocol(self, tgui):
        tgui.switch_to_spikecounts_and_set_im_combobox(
            spike_bounds_on=True, im_groupbox_on=True, im_setting="im_protocol"
        )
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_exp_lr"] in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount im protocol - upper_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["upper_bl_lr"] not in tgui.mw.loaded_file_plot.upperplot.items
        ), "Spikecount im protocol - upper_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_exp_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount im protocol - lower_exp_lr"
        assert (
            tgui.mw.spkcnt_bounds.bounds["lower_bl_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount im protocol - lower_bl_lr"
        assert (
            tgui.mw.spkcnt_bounds.axlines["lower_bl_lr"] not in tgui.mw.loaded_file_plot.lowerplot.items
        ), "Spikecount im protocol - lower bl axline"
    ...
    def test_spikecount_im_protocol_error_catch(self, tgui):
        ...
        assert self.check_im_protocol(
            tgui, "spkcnt", correctly_filled=False
    ...
    def test_spikecount_im_protocol_no_error(self, tgui):
        ...
        assert self.check_im_protocol(
            tgui, "spkcnt", correctly_filled=True
    ...
    def test_spikecount_user_im_widgets_error_catch(self, tgui):
        ...
        assert self.check_user_im_input(
            tgui, tgui.num_recs, "spkcnt", correctly_filled=False
    ...
    def test_spikecount_user_im_widgets_no_error(self, tgui):
        ...
        assert self.check_user_im_input(
            tgui, tgui.num_recs, "spkcnt", correctly_filled=True
    ...
    def test_input_resistance_im_protocol_no_error(self, tgui):
        tgui.switch_to_input_resistance_and_set_im_combobox(im_setting="im_protocol")
        assert self.check_im_protocol(
            tgui, "Ri", correctly_filled=True
        ), "Test Input Resistance user input analysis run - filled correctly"

    def test_input_resistance_user_im_error(self, tgui):
        tgui.switch_to_input_resistance_and_set_im_combobox(im_setting="user_input_im")
        assert self.check_user_im_input(tgui, tgui.num_recs, "Ri", correctly_filled=False), "test"

    def test_spkcnt_resistance_user_im_no_error(self, tgui):
        tgui.switch_to_input_resistance_and_set_im_combobox(im_setting="user_input_im")
        assert self.check_user_im_input(tgui, tgui.num_recs, "Ri", correctly_filled=True), "TEST "
    ...
    def test_spikecount_bound_limits(self, tgui):
        tgui.switch_to_spikecounts_and_set_im_combobox(spike_bounds_on=True, im_groupbox_on=True)
        self.check_bounds_link_on_and_off(
            tgui,
            tgui.mw.spkcnt_bounds,
            " Spikecount",
            ["upper_exp_lr", "lower_bl_lr", "lower_exp_lr"],
        )
    ...
    def check_bounds_dict_has_correct_number_of_recs(self, analysis_cfg, num_recs):
        for key in [
            "upper_bl_lr_lowerbound",
            "upper_bl_lr_upperbound",
            "upper_exp_lr_lowerbound",
            "upper_exp_lr_upperbound",
            "lower_bl_lr_lowerbound",
            "lower_bl_lr_upperbound",
            "lower_exp_lr_lowerbound",
            "lower_exp_lr_upperbound",
        ]:
    ...
    def check_boundary_cfgs_length_matches_num_recs(self, tgui, num_recs):
        ...
        for analysis_cfg in [
            ...
            assert self.check_bounds_dict_has_correct_number_of_recs(analysis_cfg, num_recs), (
                analysis_cfg["name"] + "analysis config boudary recs wrong"
            )
        ...
        assert self.check_bounds_dict_has_correct_number_of_recs(analysis_cfgs, num_recs), (
            analysis_cfg["name"] + "analysis config boudary recs wrong"
        )
    ...
    def test_all_bounds_are_linked_to_correct_configs(self, tgui):
        ...
        """
        for bounds, cfg_name in zip(
            [tgui.mw.spkcnt_bounds, tgui.mw.skinetics_bounds, tgui.mw.ir_bounds],
            ["spkcnt", "skinetics", "Ri"],
        ):
    ...
    def test_spkcnt_ir_skinetics_linked_across_records(self, tgui, mode):
        ...
        for filenum in range(2):
            ...
            tgui.switch_to_spikecounts_and_set_im_combobox(spike_bounds_on=True, im_groupbox_on=False)
            tgui.assign_random_boundary_position_for_every_rec_and_test(tgui, tgui.mw.spkcnt_bounds, mode)

            tgui.switch_to_input_resistance_and_set_im_combobox()
            tgui.assign_random_boundary_position_for_every_rec_and_test(tgui, tgui.mw.ir_bounds, mode)

            tgui.switch_to_skinetics_and_set_bound(skinetics_bounds_on=True)
            tgui.assign_random_boundary_position_for_every_rec_and_test(tgui, tgui.mw.skinetics_bounds, mode)
    ...
    def test_curve_fitting_linked_across_records(self, tgui, mode):
        ...
        for filenum in range(2):
            ...
            for reg in ["reg_1", "reg_2", "reg_3", "reg_4", "reg_5", "reg_6"]:
                tgui.assign_random_boundary_position_for_every_rec_and_test(
                    tgui, tgui.mw.curve_fitting_regions[reg], mode
                )
    ...
    @pytest.mark.parametrize(
        "mode",
        [
            "link_im_vm_ON_and_link_recs_ON",
            "link_im_vm_OFF_and_link_recs_ON",
            "link_im_vm_ON_and_link_recs_OFF",
            "link_im_vm_OFF_and_link_recs_OFF",
        ],
    )
        ...
        for filenum in range(2):
            ...
            for bound_pair in [
                ...
                for rec in range(tgui.mw.loaded_file.data.num_recs):
                    ...
                    all_start.append(
                        start_time - tgui.mw.loaded_file.data.min_max_time[rec][0]
                    )  # save the normalised version that matches configs
    ...
    def test_spikecounts_im_baseline_is_correct(self, tgui):
        ...
        average_bounds, average_axline, test_average = self.check_lr_baseline(
            tgui,
            tgui.mw.spkcnt_bounds,
            "lower_bl_lr",
    ...
    def test_input_resistance_im_lower_exp_is_correct(self, tgui):
        tgui.switch_to_input_resistance_and_set_im_combobox()
        average_bounds, __, test_average = self.check_lr_baseline(
            tgui,
            tgui.mw.ir_bounds,
            "lower_exp_lr",
            tgui.mw.loaded_file.data.im_array,
    ...
    def test_spikecount_user_im_widgets_error_catch_within_recs(self, tgui):
        ...
        num_recs = tgui.set_analyse_specific_recs(rec_from=5, rec_to=10)
        assert self.check_user_im_input(
            tgui, num_recs, "spkcnt", correctly_filled=False, within_recs=True
        ), "Test Spikecount user input analysis run - filled incorrectly"
    ...
    def test_spikecount_user_im_widgets_no_error_within_recs(self, tgui):
        ...
        num_recs = tgui.set_analyse_specific_recs(rec_from=5, rec_to=10)
        assert self.check_user_im_input(
            tgui, num_recs, "spkcnt", correctly_filled=True
        ), "Test Spikecount user input analysis run - filled correctly"

    def test_input_resistance_user_im_error_within_recs(self, tgui):
        tgui.switch_to_input_resistance_and_set_im_combobox(im_setting="user_input_im")
        num_recs = tgui.set_analyse_specific_recs(rec_from=5, rec_to=10)
        assert self.check_user_im_input(tgui, num_recs, "Ri", correctly_filled=False), "test"
    ...
    def test_spikecount_boundaries_within_analysed_recs(self, tgui):
        tgui.switch_to_spikecounts_and_set_im_combobox(spike_bounds_on=True, im_groupbox_on=True)
        _ = tgui.set_analyse_specific_recs(rec_from=5, rec_to=10)
