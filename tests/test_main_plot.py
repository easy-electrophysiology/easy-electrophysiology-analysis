...
class TestLinearRegionsImSettings:
    ...
    def get_limits(self, tgui, rec):
        ...
        pad_left = (
            tgui.mw.loaded_file.data.min_max_time[rec][1] - tgui.mw.loaded_file.data.min_max_time[rec][0]
        ) * vb_pad_left  # small offset added to end of viewbox set exactly to range cuts off last xticklabel
    ...
    def test_x_axis_on_scroll(self, tgui):
        for rec in range(0, tgui.adata.num_recs):
            ...
            assert np.isclose(
                upper_min,
                tgui.mw.loaded_file.data.min_max_time[rec][0],
                atol=1e-08,
                rtol=0,
            ), "error in upper min rec:{0}".format(rec)
            assert np.isclose(
                upper_max,
                tgui.mw.loaded_file.data.min_max_time[rec][1],
                atol=1e-08,
                rtol=0,
            ), "error in upper max rec:{0}".format(rec)
            assert np.isclose(
                lower_min,
                tgui.mw.loaded_file.data.min_max_time[rec][0],
                atol=1e-08,
                rtol=0,
            ), "error in lower min rec:{0}".format(rec)
            assert np.isclose(
                lower_max,
                tgui.mw.loaded_file.data.min_max_time[rec][1],
                atol=1e-08,
                rtol=0,
            ), "error in lower max rec:{0}".format(rec)
