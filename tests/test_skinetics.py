...
class TestsKinetics:
    ...
    def test_skinetics_decay_time(self, tsine, freq, amplitude, min_cutoff, max_cutoff, thr_type):
        ...
        assert self.check_close(expected_max_time / freq - expected_min_time / freq, decay_time, ts)
    ...
    def test_skinetics_fahp_mahp(self, tsine, freq, amplitude):
        ...
        fahp_time, fahp_vm, fahp_idx, fahp = core_analysis_methods.calculate_ahp(
            data, 0, search_period, sin_y, sin_x, idxs["max"], 0
        )
    ...
    def test_threshold(self, cfgs, l):
        ...
        method_ii_idx = core_analysis_methods.calculate_threshold(
            first_deriv, second_deriv, third_deriv, 0, 10000, cfgs
        )
    ...
    def test_phase_plot_params(self):
        ...
        test_vmax_x, test_vmax_x_diff = current_calc.calculate_vmax(
            x, x_diff
        )  # the last vm is cut off because of diff(). Under normal circumstances, this will have no effect for AP analysis
        assert test_vmax_x == 169
        assert test_vmax_x_diff == -5

        (
            test_vm_diff_min_x,
            test_vm_diff_min_x_diff,
        ) = current_calc.calculate_vm_diff_min(x, x_diff)
        assert test_vm_diff_min_x == 149
        assert test_vm_diff_min_x_diff == -100

        (
            test_vm_diff_max_x,
            test_vm_diff_max_x_diff,
        ) = current_calc.calculate_vm_diff_max(x, x_diff)
