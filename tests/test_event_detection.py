...
# ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def setup_test_events_detection(self, tgui):
        ...
        run_settings = {"fs": 1 / ts, "deconv_options": {"filt_low_hz": 1, "filt_high_hz": 200}, "direction": -1, "ts": ts, "window_len_s": 0.01, "rise_s": 0.0005, "decay_s": 0.005}
        ...
        template = core_analysis_methods.biexp_event_function(x,  (0, -1, run_settings["rise_s"], run_settings["decay_s"]))
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def test_correlation_and_detection_criterion(self, tgui):
        ...
        detection_crit, betas, r_ = voltage_calc.clements_bekkers_sliding_window(data[0], template, progress_bar_callback)

        test_r, test_betas = test_correlation.sliding_window(data,
                                                             run_settings["window_len_s"],
                                                             run_settings["rise_s"],
        ...
        test_detection_crit, test_scale, test_offset = test_correlation.clements_bekkers(data[0], template)
        ...
        stf_detection_crit = np.load(os.path.realpath(os.path.dirname(__file__) + "/stf_detection_crit.npy"))
        stf_correlation = np.load(os.path.realpath(os.path.dirname(__file__) + "/stf_correlation.npy"))
