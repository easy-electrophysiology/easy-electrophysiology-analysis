...
from easy_electrophysiology.mainwindow.exception_hook import dump_analysis_after_software_crash
...
class TestBackupOptions:
    ...
    def check_events_analysis(self, saved_events, events_data):  # TODO: break into this for final double check
        for col, key in enumerate(
            ...
            assert np.array_equal(
                saved_events.iloc[1:, col].to_numpy().astype(np.float64),
                events_data[key],
            )
    ...
    def test_all_analysis_are_saved_correctly(self, tgui):
        ...
        ]  # save for the checking function later
        ...
        test_curve_fitting.run_curve_fitting(
            tgui,
            vary_coefs=False,
            func_type="max",
            region_name="reg_1",
        ...
        tgui.test_saved_input_resistance_to_loaded_file_data(
            saved_input_resistance_data, input_resistance_data, ir_rec_from, ir_rec_to
        )

        saved_skinetics_data = pd.read_csv(backup_path + "/ap_kinetics.csv")
        for col in range(skinetics_data_table.shape[1]):
            assert np.array_equal(
                saved_skinetics_data.iloc[1:, 2 + col].to_numpy().astype(np.float64),
                skinetics_data_table[:, col],
            )

        saved_curve_fitting = pd.read_csv(backup_path + "/curve_fitting.csv")
        for col, key in enumerate(["event_num", "baseline", "max", "amplitude"]):
            assert np.array_equal(
                saved_curve_fitting.iloc[1:, col].to_numpy().astype(np.float64),
                curve_fitting_data[key],
            )
