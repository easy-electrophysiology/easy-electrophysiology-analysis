...
from ephys_data_methods.importdata import check_loaded_files_match
...
class TestConfigs:
    def __init__(self):
        self.file_load_options = {
            "select_channels_to_load": {"on": False},
...
def make_path(file_ext, filename):
    base_dir = os.path.join(os.path.dirname(__file__), "data", "importdata_tests")
...
class TestImportData:
    ...
    def cc_one_channel_abf_info_dict(self):
        cc_one_channel_abf_info_dict = dict(
            num_recs=7,
            num_samples=20000,
            num_data_channels=1,
            analysis_type="current_clamp",
            t_start=0,
            t_stop=182,
            vm_units="mV",
            im_units=None,
    ...
    def cc_two_channels_abf_info_dict(self):
        cc_two_channels_abf_info_dict = dict(
            num_recs=12,
            num_samples=140000,
            num_data_channels=2,
            analysis_type="current_clamp",
    ...
    def cc_one_channel_wcp_info_dict(self):
        cc_one_channel_wcp_info_dict = dict(
            num_recs=7,
            num_samples=20224,
            num_data_channels=1,
            analysis_type="current_clamp",  # weird, WinWCP seems to be resampling the ABF
            t_start=0,
            t_stop=2.0223999489098787,
            vm_units="mV",
            im_units=None,
            time_units="s",
            time_offset=0,
    ...
    def cc_two_channels_wcp_info_dict(self):
        cc_two_channels_wcp_info_dict = dict(
            num_recs=15,
            num_samples=65536,
            num_data_channels=2,
            analysis_type="current_clamp",
            t_start=0,
            t_stop=2.998271942138672,
            vm_units="mV",
            im_units="pA",
            time_units="s",
            time_offset=0,
    ...
    def vc_one_channel_edr_info_dict(self):
        cc_two_channels_wcp_info_dict = dict(
            ...
            num_samples=120000,
            num_data_channels=1,
            analysis_type="voltage_clamp_1_record",
            t_start=0,
            t_stop=120,
            vm_units=None,
            im_units="pA",
    ...
    def vc_two_channels_info_dict(self):
        vc_two_channels_info_dict = dict(
            num_recs=40,
            num_samples=2048,
            num_data_channels=2,
            analysis_type="voltage_clamp_multi_record",
            t_start=0,
            t_stop=0.9999359846115111,
            vm_units="mV",
            im_units="pA",
            time_units="s",
            time_offset=0,
    ...
    def cc_one_channel_axgd_info_dict(self):
        cc_one_channel_axgd_info_dict = dict(
            num_recs=39,
            num_samples=40000,
            num_data_channels=1,
            analysis_type="current_clamp",
            t_start=0.00002,
            t_stop=0.8000200000000002,
            vm_units="mV",
            im_units=None,
            time_units="s",
            time_offset=0.00002,
    ...
    def vc_multi_channel_axograph_info_dict(self):
        vc_multi_channel_axograph_info_dict = dict(
            num_recs=1,
            num_samples=1000,
            num_data_channels=1,
            analysis_type="voltage_clamp_1_record",
            t_start=0.00005,
            t_stop=0.050050000000000004,
            vm_units=None,
            im_units="pA",
            time_units="s",
            time_offset=0.00005,
    ...
    def load_file_with_neo(fixture_path_lists, cfgs, load_data=False, mw=None, return_class=False):
        ...
        if return_class:
            return (
                import_data.channels,
                import_data.reader,
                import_data.neo_block,
                file_ext,
                import_data,
            )
        else:
    ...
    @staticmethod
    def get_info_dict_from_fileinfo(
        filename,
        file_ext,
        cc_one_channel_abf_info_dict,
        cc_two_channels_abf_info_dict,
        cc_one_channel_wcp_info_dict,
        cc_two_channels_wcp_info_dict,
        vc_one_channel_edr_info_dict,
        vc_two_channels_info_dict,
        cc_one_channel_axgd_info_dict,
        vc_multi_channel_axgx_info_dict,
    ):
    ...
    def test_cc_two_channels_get_channel_info(self, cc_two_channels_path, cfgs):
        ...
        """
        channels, reader, __, file_ext, import_data = self.load_file_with_neo(
            cc_two_channels_path, cfgs, return_class=True
        )
        (
            channel_1,
            channel_1_idx,
            channel_2,
            channel_2_idx,
        ) = import_data.get_channel_data_info()
    ...
    def test_cc_one_channel_get_channel_info(self, cc_one_channel_path, cfgs):
        channels, reader, __, file_ext, import_data = self.load_file_with_neo(
            cc_one_channel_path, cfgs, return_class=True
        )
        (
            channel_1,
            channel_1_idx,
    ...
    def test_vc_two_channels_get_channel_info(self, vc_two_channels_path, cfgs):
        channels, reader, __, file_ext, import_data = self.load_file_with_neo(
            vc_two_channels_path, cfgs, return_class=True
        )
        (
            channel_1,
            channel_1_idx,
    ...
    def test_vc_one_channel_edr_get_channel_info(self, vc_one_channel_edr, cfgs):
        channels, reader, __, file_ext, import_data = self.load_file_with_neo(
            vc_one_channel_edr, cfgs, return_class=True
        )
        (
            channel_1,
            channel_1_idx,
    ...
    def test_cc_one_channel_axgd_get_channel_info(self, cc_one_channel_axgd, cfgs):
        channels, reader, __, file_ext, import_data = self.load_file_with_neo(
            cc_one_channel_axgd, cfgs, return_class=True
        )
        (
            channel_1,
            channel_1_idx,
    ...
    def test_vc_multi_channel_axgx_get_channel_info(self, vc_multi_channel_axgx, cfgs):
        ...
        channels, reader, __, file_ext, import_data = self.load_file_with_neo(
            vc_multi_channel_axgx, cfgs, return_class=True
        )
        (
            channel_1,
            channel_1_idx,
    ...
    def test_get_channel_type(
        self,
        cc_two_channels_path,
        cc_one_channel_path,
        vc_two_channels_path,
        vc_one_channel_edr,
        cfgs,
    ):
        """
        ...
        for full_filepath, type_ in zip(
            [
                cc_two_channels_path,
                cc_one_channel_path,
                vc_two_channels_path,
            ...
            channels, reader, __, file_ext, import_data = self.load_file_with_neo(
                full_filepath, cfgs, return_class=True
            )
            (
                channel_1,
                channel_1_idx,
    ...
    def test_importdata_all_files(
        ...
        vc_two_channels_info_dict,
        cc_one_channel_axgd_info_dict,
        vc_multi_channel_axograph_info_dict,
        cfgs,
    ):
        """
        ...
        info_dict = self.get_info_dict_from_fileinfo(
            filename,
            file_ext,
            cc_one_channel_abf_info_dict,
            cc_two_channels_abf_info_dict,
        ...
        channels, reader, __, file_ext, import_data = self.load_file_with_neo(
            [full_filepath, file_ext], cfgs, return_class=True
        )
        ...
        assert imported_data.num_data_channels == info_dict["num_data_channels"], (
            "data channels: " + filename + file_ext
        )
    ...
    def test_extract_data_vm_from_array(self, artificial_neo_block):
        importdata_vm_array = importdata.RawData.extract_data_from_array(
            self=artificial_neo_block,
            data_idx=artificial_neo_block.vm_idx,
            neo_block=artificial_neo_block,
        )
        assert np.array_equal(importdata_vm_array, artificial_neo_block.test_vm_array), "extract data vm"

    def test_extract_data_im_from_array(self, artificial_neo_block):
        importdata_im_array = importdata.RawData.extract_data_from_array(
            self=artificial_neo_block,
            data_idx=artificial_neo_block.im_idx,
            neo_block=artificial_neo_block,
        )
        assert np.array_equal(importdata_im_array, artificial_neo_block.test_im_array), "extract data vm"

    def test_extract_time_array(self, artificial_neo_block):
        importdata_time_array, t_start, t_stop = importdata.RawData.extract_time_array(
            self=artificial_neo_block, neo_block=artificial_neo_block
        )

        assert utils.allclose(
            artificial_neo_block.test_time_array.magnitude, importdata_time_array, 1e-8
        ), "extract data time"
    ...
    def test_wavesurfer_sweep_order_unit(self):
        """
        Test a few variations of possible sweep inputs for wavesurfer
        and check that they are always ordered correctly.
        """
        from neo.io.wavesurferio import get_ordered_list_of_sweeps

        test_1 = ["sweep_005", "sweep_004", "sweep_001", "sweep_000", "header"]
        assert get_ordered_list_of_sweeps(test_1) == [
            "sweep_000",
            "sweep_001",
            "sweep_004",
            "sweep_005",
        ...
        assert get_ordered_list_of_sweeps(test_2) == [
            "sweep_1",
            "sweep_05",
            "sweep_6",
            "sweep_007",
        ...
        test_3 = ["sweep_" + str(num).zfill(4) for num in range(11111)]
        correct_order_test_3 = copy.deepcopy(test_3)

        test_3 += ["header"]
        random.shuffle(test_3)
        assert get_ordered_list_of_sweeps(test_3) == correct_order_test_3

        test_4 = ["sweep_0005", "sweep_0004", "header"]
        assert get_ordered_list_of_sweeps(test_4) == ["sweep_0004", "sweep_0005"]

    # ----------------------------------------------------------------------------------------------------------------------------------------------------
    # Multi-File Loading Bad Cases
    # ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def test_all_bad_cases(self, method):
        """"""
        artificial_neo_block_1 = ArtificialNeoBlock()
        artificial_neo_block_2 = ArtificialNeoBlock()

        self.set_default_attributes_to_same(artificial_neo_block_1, artificial_neo_block_2)

        assert check_loaded_files_match(artificial_neo_block_1, artificial_neo_block_2)

        setattr(artificial_neo_block_1, method, "a")
        setattr(artificial_neo_block_2, method, "b")
        assert [method, "b", "a"] == check_loaded_files_match(artificial_neo_block_1, artificial_neo_block_2)

    def set_default_attributes_to_same(self, artificial_neo_block_1, artificial_neo_block_2):
        """ """
        setattr(artificial_neo_block_1, "recording_type", "current_clamp")
        setattr(artificial_neo_block_2, "recording_type", "current_clamp")

        setattr(artificial_neo_block_1, "channel_1_type", "Vm")
        setattr(artificial_neo_block_2, "channel_1_type", "Vm")

        setattr(artificial_neo_block_1, "channel_2_type", "Im")
        setattr(artificial_neo_block_2, "channel_2_type", "Im")

        setattr(artificial_neo_block_1, "vm_units", "mV")
        setattr(artificial_neo_block_2, "vm_units", "mV")

        setattr(artificial_neo_block_1, "im_units", "pA")
        setattr(artificial_neo_block_2, "im_units", "pA")

        setattr(artificial_neo_block_1, "time_offset", 0)
        setattr(artificial_neo_block_2, "time_offset", 0)

        setattr(artificial_neo_block_1, "time_offset", 0)
        setattr(artificial_neo_block_2, "time_offset", 0)

        setattr(artificial_neo_block_1, "time_units", "s")
        setattr(artificial_neo_block_2, "time_units", "s")
    ...
    def test_time_on_tagged_file(self):
        ...
        for rec in range(tgui.mw.loaded_file.data.num_recs):
            ...
            assert np.isclose(rec_time[-1] - rec_time[0], 0.4999, atol=1e-10, rtol=0)
