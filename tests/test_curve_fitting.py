...
def run_curve_fitting(
    ...
    tgui.update_curve_fitting_function(
        vary_coefs, insert_function, norm_or_cumu_time, pos_or_neg
...
def setup_and_run_curve_fitting_analysis(
    tgui, func_type, region_name, rec_from, rec_to, set_options_only, pos_or_neg="pos"
):
    """
    ...
    if region_name == "all":
        for reg in ["reg_1", "reg_2", "reg_3", "reg_4", "reg_5", "reg_6"]:
            ...
            setup_gui_for_analysis_and_run(
                tgui,
                reg,
                func_type,
                rec_from,
                rec_to,
                set_options_only=set_options_only,
    ...
    # Why is this deleting?
    # tgui.mw.dialogs["curve_fitting"].setVisible(False)


def setup_gui_for_analysis_and_run(
    tgui,
    region_name,
    func_type,
    rec_from,
    ...
    tgui.set_combobox(dialog.dia.fit_type_combobox, idx=get_combobox_idx_from_analysis_type(func_type))
    tgui.switch_checkbox(dialog.dia.link_and_unlink_curve_fit_region_checkbox, on=False)
    ...
    for rec in range(tgui.adata.num_recs):
        ...
        tgui.mw.curve_fitting_regions[region_name].bounds["upper_exp_lr"].setRegion(
            (
                tgui.adata.start_times[
                    rec
                ],  # confirmed LR is not on plot, but the bounds are still updated because this is not a condition
                tgui.adata.stop_times[rec],
            )
        )  # in curve_fitting_linear_regions.py update_bounds_after_mw_change() wheras it is is linear_regions.py for spkcnt, skinetics and Ri
        tgui.mw.curve_fitting_regions[region_name].bounds["upper_bl_lr"].setRegion((0, tgui.adata.start_times[rec] - 1))
...
def set_direction_radiobutton(tgui, func_type, dialog, pos_or_neg):
    radiobuttons = {
        "monoexp": {
            "pos": dialog.dia.monoexp_direction_pos_radiobutton,
            "neg": dialog.dia.monoexp_direction_neg_radiobutton,
        },
        "biexp_decay": {
            "pos": dialog.dia.biexp_decay_direction_pos_radiobutton,
            "neg": dialog.dia.biexp_decay_direction_neg_radiobutton,
        },
        "biexp_event": {
            "pos": dialog.dia.biexp_event_direction_pos_radiobutton,
            "neg": dialog.dia.biexp_event_direction_neg_radiobutton,
        },
        "triexp": {
            "pos": dialog.dia.triexp_direction_pos_radiobutton,
...
def override_biexp_decay_coefs(tgui, rec_from, rec_to, rec=0):
    ...
    tgui.switch_groupbox(dialog.biexp_decay_coefficients_groupbox, on=True)
    tgui.enter_number_into_spinbox(dialog.biexp_decay_initial_est_b0_spinbox, str(tgui.adata.offset))
    tgui.enter_number_into_spinbox(
        dialog.biexp_decay_initial_est_b1_spinbox,
        str(tgui.adata.get_b_coef("biexp_decay", "b1", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.biexp_decay_initial_est_tau1_spinbox,
        str(tgui.adata.get_tau("biexp_decay", "tau1", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.biexp_decay_initial_est_b2_spinbox,
        str(tgui.adata.get_b_coef("biexp_decay", "b2", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.biexp_decay_initial_est_tau2_spinbox,
...
def override_tripex_coefs(tgui, rec_from, rec_to, rec=0):
    ...
    tgui.switch_groupbox(dialog.triexp_coefficients_groupbox, on=True)

    tgui.enter_number_into_spinbox(dialog.triexp_initial_est_b0_spinbox, str(tgui.adata.offset))
    tgui.enter_number_into_spinbox(
        dialog.triexp_initial_est_b1_spinbox,
        str(tgui.adata.get_b_coef("triexp", "b1", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.triexp_initial_est_tau1_spinbox,
        str(tgui.adata.get_tau("triexp", "tau1", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.triexp_initial_est_b2_spinbox,
        str(tgui.adata.get_b_coef("triexp", "b2", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.triexp_initial_est_tau2_spinbox,
        str(tgui.adata.get_tau("triexp", "tau2", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.triexp_initial_est_b3_spinbox,
        str(tgui.adata.get_b_coef("triexp", "b3", rec_from, rec_to, rec)),
    )
    tgui.enter_number_into_spinbox(
        dialog.triexp_initial_est_tau3_spinbox,
...
def fill_in_starting_estimate(tgui, func_type, rec_from=None, rec_to=None, rec=0, analyse_one_rec_only=False):
    ...
    if analyse_one_rec_only:
        tgui.set_analyse_specific_recs(rec, rec + 1)
...
class TestCurveFitting:
    ...
    def check_qtable_peak_baseline_amplitude(self, tgui, test_peak_ims, func_type, rec_from, rec_to):
        ...
        assert np.array_equal(
            test_peak_ims,
            tgui.get_data_from_qtable("peak", rec_from, rec_to, "curve_fitting"),
        ), (
            "peak " + func_type
        )

        assert utils.allclose(
            tgui.adata.get_b0(rec_from, rec_to),
        ...
        assert utils.allclose(
            test_peak_ims - tgui.adata.offset,
    ...
    def check_triexp_coefficients_across_recs(self, tgui, data, func_type, rec_from, rec_to, rec):
        ...
        test_sum_tau = (
            tgui.adata.get_tau(func_type, "tau1", rec_from, rec_to, rec)
            + tgui.adata.get_tau(func_type, "tau2", rec_from, rec_to, rec)
            + tgui.adata.get_tau(func_type, "tau3", rec_from, rec_to, rec)
        )

        test_fit = core_analysis_methods.triexp_decay_function(
            np.arange(tgui.adata.function_samples),
            tgui.adata.cannonical_coefs[func_type] * tgui.adata.coef_offsets[rec][func_type],
        )

        test = (
            utils.allclose(sum_b, test_sum_b, 1e-05)
            or utils.allclose(sum_tau, test_sum_tau, 1e-05)
            or utils.allclose(data["fit"][:-1], test_fit, 1e-05)
        )
    ...
    def check_plots(
        ...
        if np.any(y_line) and np.any(x_line):
            self.check_data_against_plot(
                region_name,
                tgui.mw.loaded_file_plot.curve_fitting_line_plots,
                y_line,
                x_line,
            )
        ...
        if np.any(y_scatter) and np.any(x_scatter):
            self.check_data_against_plot(
                region_name,
                tgui.mw.loaded_file_plot.curve_fitting_scatter_plots,
                y_scatter,
                x_scatter,
            )
    ...
    def test_curve_fitting_model_with_changing_coefficients_within_a_function(
        self,
        tgui,
        func_type,
        vary_coefs,
        ...
        for filenum in range(3):
            rec_from, rec_to = run_curve_fitting(
                tgui,
                vary_coefs,
                func_type,
                region_name,
            ...
            for rec in range(tgui.adata.num_recs):
                ...
                for data in [
                    tgui.mw.loaded_file.curve_fitting_results[region_name]["data"][rec],
                    ...
                    assert utils.allclose(
                        data["b0"], tgui.adata.get_b0(rec_from, rec_to, rec), 1e-08
                    ), "b0"  # actually first not used
                    ...
                    if func_type == "monoexp":
                        ...
                        assert utils.allclose(
                            data["tau"],
                            tgui.adata.get_tau(func_type, "tau", rec_from, rec_to, rec),
                            1e-08,
                        ), "tau"
                        assert data["r2"] == 1
                    ...
                    if func_type == "biexp_decay":
                        peak_im = tgui.adata.get_b_coef(func_type, "b1", rec_from, rec_to, rec) + tgui.adata.get_b_coef(
                            func_type, "b2", rec_from, rec_to, rec
                        )
                    ...
                    if func_type == "biexp_event":
                        ...
                        assert utils.allclose(
                            data["fit_rise"],
                            tgui.adata.get_tau(func_type, "fit_rise", rec_from, rec_to, rec),
                            1e-05,
                        ), "fit rise"
                        assert utils.allclose(
                            data["fit_decay"],
                            tgui.adata.get_tau(func_type, "fit_decay", rec_from, rec_to, rec),
                            1e-05,
                        ), "fit decay"
                    ...
                    if func_type == "triexp":
                        ...
                        assert np.isclose(data["r2"], 1, rtol=0, atol=1e-06)

                    test_peak_im = peak_im + tgui.adata.offset
                    assert data["peak"] == test_peak_im, "peak"
                    assert (
                        data["peak_time"] == tgui.adata.start_times[rec]
                    ), "peak_time"  # not actually on the table at this time.
    ...
    def test_curve_fitting_difference_in_amplitude_across_records(
        self, tgui, func_type, analyse_specific_recs, region_name, pos_or_neg
    ):
        """
            ...
            for rec in range(tgui.adata.num_recs):
                ...
                for data in [
                    ...
                    if func_type == "biexp_decay":
                        ...
                        sum_test_tau = tgui.adata.get_tau(
                            func_type, "tau1", rec_from, rec_to, rec
                        ) + tgui.adata.get_tau(func_type, "tau2", rec_from, rec_to, rec)

                        test_fit = core_analysis_methods.biexp_decay_function(
                            np.arange(tgui.adata.function_samples),
                            tgui.adata.cannonical_coefs["biexp_decay"] * tgui.adata.coef_offsets[rec][func_type],
                        )
                        test = (
                            utils.allclose(sum_b, sum_test_b, 1e-05)
                            or utils.allclose(sum_tau, sum_test_tau, 1e-05)
                            or utils.allclose(data["fit"][:-1], test_fit, 1e-05)
                        )
                    ...
                    if func_type == "triexp":
                        test, peak_im = self.check_triexp_coefficients_across_recs(
                            tgui, data, func_type, rec_from, rec_to, rec
                        )
    ...
    def test_analyse_all_vs_analyse_single_region(self, tgui):
        ...
        assert utils.allclose(
            data["tau"],
            tgui.adata.get_tau("monoexp", "tau", rec_from, rec_to, rec),
            1e-08,
        ), "tau"
    ...
    def test_qtable_num_recs_peak_amplitude_baseline_for_all_func_types(
        self, tgui, func_type, vary_coefs, analyse_specific_recs, region_name
    ):
        """
        ...
        assert np.array_equal(table_recs, (np.arange(rec_from, rec_to + 1) + 1)), "table recs"

        # Test Peaks / Amplitude / Baseline
        if func_type == "monoexp":
            peak_ims = tgui.adata.get_b_coef(func_type, "b1", rec_from, rec_to)
        elif func_type == "biexp_decay":
            peak_ims = tgui.adata.get_b_coef(func_type, "b1", rec_from, rec_to) + tgui.adata.get_b_coef(
                func_type, "b2", rec_from, rec_to
            )
    ...
    @pytest.mark.parametrize("analyse_specific_recs", [False, True])
    @pytest.mark.parametrize("region_name", REGIONS_TO_RUN)
    def test_monoexp_qtable_with_changing_coefficients_within_a_function(
        self, tgui, vary_coefs, analyse_specific_recs, region_name
    ):
        """ """
        rec_from, rec_to = run_curve_fitting(
            tgui,
        ...
        assert utils.allclose(
            tgui.adata.get_b_coef("monoexp", "b1", rec_from, rec_to),
            tgui.get_data_from_qtable("b1", rec_from, rec_to, "curve_fitting"),
            1e-08,
        ), "b1"

        assert utils.allclose(
            tgui.adata.get_tau("monoexp", "tau", rec_from, rec_to),
    ...
    @pytest.mark.parametrize("analyse_specific_recs", [False, True])
    @pytest.mark.parametrize("region_name", REGIONS_TO_RUN)
    def test_biexp_decay_qtable_with_changing_coefficients_within_a_function(
        self, tgui, vary_coefs, analyse_specific_recs, region_name
    ):
        """ """
        rec_from, rec_to = run_curve_fitting(
        ...
        assert utils.allclose(
            tgui.adata.get_b_coef("biexp_decay", "b1", rec_from, rec_to),
            tgui.get_data_from_qtable("b1", rec_from, rec_to, "curve_fitting"),
            1e-0,
        ), "b1"

        assert utils.allclose(
            tgui.adata.get_tau("biexp_decay", "tau1", rec_from, rec_to),
            tgui.get_data_from_qtable("tau1", rec_from, rec_to, "curve_fitting"),
            1e-2,
        ), "tau1"

        assert utils.allclose(
            tgui.adata.get_b_coef("biexp_decay", "b2", rec_from, rec_to),
            tgui.get_data_from_qtable("b2", rec_from, rec_to, "curve_fitting"),
            1e-0,
        ), "b2"

        assert utils.allclose(
            tgui.adata.get_tau("biexp_decay", "tau2", rec_from, rec_to),
    ...
    @pytest.mark.parametrize("analyse_specific_recs", [False, True])
    @pytest.mark.parametrize("region_name", REGIONS_TO_RUN)
    def test_biexp_event_qtable_with_changing_coefficients_within_a_function(
        self, tgui, vary_coefs, analyse_specific_recs, region_name
    ):
        """ """
        rec_from, rec_to = run_curve_fitting(
        ...
        assert utils.allclose(
            tgui.adata.get_b_coef("biexp_event", "b1", rec_from, rec_to),
            tgui.get_data_from_qtable("b1", rec_from, rec_to, "curve_fitting"),
            1e-05,
        ), "b1"

        assert utils.allclose(
            tgui.adata.get_tau("biexp_event", "fit_rise", rec_from, rec_to),
            tgui.get_data_from_qtable("fit_rise", rec_from, rec_to, "curve_fitting"),
            1e-05,
        ), "fit rise"

        assert utils.allclose(
            tgui.adata.get_tau("biexp_event", "fit_decay", rec_from, rec_to),
    ...
    @pytest.mark.parametrize("analyse_specific_recs", [False, True])
    @pytest.mark.parametrize("region_name", REGIONS_TO_RUN)
    def test_triexp_qtable_with_changing_coefficients_within_a_function(
        self, tgui, vary_coefs, analyse_specific_recs, region_name
    ):
        """ """
        rec_from, rec_to = run_curve_fitting(
        ...
        assert utils.allclose(
            tgui.adata.get_b_coef("triexp", "b1", rec_from, rec_to),
            tgui.get_data_from_qtable("b1", rec_from, rec_to, "curve_fitting"),
            1e-00,
        ), "b1"

        assert utils.allclose(
            tgui.adata.get_tau("triexp", "tau1", rec_from, rec_to),
            tgui.get_data_from_qtable("tau1", rec_from, rec_to, "curve_fitting"),
            1e-02,
        ), "tau1"

        assert utils.allclose(
            tgui.adata.get_b_coef("triexp", "b2", rec_from, rec_to),
            tgui.get_data_from_qtable("b2", rec_from, rec_to, "curve_fitting"),
            1e-00,
        ), "b2"  # 1dp for triexp coefs

        assert utils.allclose(
            tgui.adata.get_tau("triexp", "tau2", rec_from, rec_to),
            tgui.get_data_from_qtable("tau2", rec_from, rec_to, "curve_fitting"),
            1e-02,
        ), "tau2"

        assert utils.allclose(
            tgui.adata.get_b_coef("triexp", "b3", rec_from, rec_to),
            tgui.get_data_from_qtable("b3", rec_from, rec_to, "curve_fitting"),
            1e-00,
        ), "b3"

        assert utils.allclose(
            tgui.adata.get_tau("triexp", "tau3", rec_from, rec_to),
    ...
    def test_all_recs_analysed_together_curve_fit(self, tgui, func_type, analyse_specific_recs):
        ...
        # Table Recs
        table_recs = tgui.get_data_from_qtable("record_num", rec_from, rec_to, "curve_fitting", return_regions=True)
        assert self.check_all_sublists_are_equal(table_recs), "table recs all regions equal"
        assert np.array_equal(table_recs[0], (np.arange(rec_from, rec_to + 1) + 1)), "table recs"

        # B0 -----------------------------------------------------------------------------------------------------------------------------------------

        b0_results = tgui.get_data_from_qtable("b0", rec_from, rec_to, "curve_fitting", return_regions=True)
        ...
        b1_results = tgui.get_data_from_qtable("b1", rec_from, rec_to, "curve_fitting", return_regions=True)
        ...
        if func_type == "monoexp":
            tau_results = tgui.get_data_from_qtable("tau", rec_from, rec_to, "curve_fitting", return_regions=True)
        ...
        if func_type == "biexp_event":
            rise_results = tgui.get_data_from_qtable("fit_rise", rec_from, rec_to, "curve_fitting", return_regions=True)
            ...
            decay_results = tgui.get_data_from_qtable(
                "fit_decay", rec_from, rec_to, "curve_fitting", return_regions=True
            )
        ...
        if func_type == "biexp_decay" or func_type == "triexp":
            tau1_results = tgui.get_data_from_qtable("tau1", rec_from, rec_to, "curve_fitting", return_regions=True)
            ...
            b2_results = tgui.get_data_from_qtable("b2", rec_from, rec_to, "curve_fitting", return_regions=True)
            assert self.check_all_sublists_are_equal(b2_results), "b2 all regions equal"
            assert utils.allclose(
                b2_results[0],
                tgui.adata.get_b_coef(func_type, "b2", rec_from, rec_to),
                1e-00,
            ), "b2"  # not sure why this only 1 dp for triexp

            tau2_results = tgui.get_data_from_qtable("tau2", rec_from, rec_to, "curve_fitting", return_regions=True)
        ...
        if func_type == "triexp":
            b3_results = tgui.get_data_from_qtable("b3", rec_from, rec_to, "curve_fitting", return_regions=True)
            ...
            tau3_results = tgui.get_data_from_qtable("tau3", rec_from, rec_to, "curve_fitting", return_regions=True)
    ...
    def test_min_max_mean_median_and_plots(self, tgui, func_type, vary_coefs, analyse_specific_recs, region_name):
        ...
        for rec in range(tgui.adata.num_recs):
            ...
            for data in [
                ...
                inserted_function = tgui.mw.loaded_file.data.im_array[rec][
                    tgui.adata.start_sample : tgui.adata.stop_sample + 1
                ]
                inserted_function_time_period = tgui.mw.loaded_file.data.time_array[rec][
                    tgui.adata.start_sample : tgui.adata.stop_sample + 1
                ]
                ...
                if func_type == "mean":
                    ...
                    self.check_plots(
                        tgui,
                        region_name,
                        y_line=np.tile(data["mean"], len(data["fit_time"])),
                        x_line=data["fit_time"],
                    )
                ...
                if func_type == "median":
                    ...
                    self.check_plots(
                        tgui,
                        region_name,
                        y_line=np.tile(data["median"], len(data["fit_time"])),
                        x_line=data["fit_time"],
                    )

                if func_type == "area_under_curve_cf":
                    self.check_area_under_curve_and_plots(
                        tgui,
                        data,
                        region_name,
                        inserted_function,
                        inserted_function_time_period,
                    )
    ...
    def get_inserted_function_peak(self, func_type, inserted_function, axis=1):
        ...
        if func_type in ["min", "max"]:
            peak_im = (
                np.max(inserted_function, axis=axis) if func_type != "min" else np.min(inserted_function, axis=axis)
            )
        else:
            peak_im = (
                np.mean(inserted_function, axis=axis)
                if func_type == "mean"
                else np.median(inserted_function, axis=axis)
            )
        return peak_im

    def check_area_under_curve_and_plots(
        self, tgui, data, region_name, inserted_function, inserted_function_time_period
    ):
        """
        ...
        assert np.array_equal(
            baseline_curve.xData, inserted_function_time_period
        ), "inserted_function_time_period baseline_curve"
        ...
        assert np.array_equal(
            data_curve.xData, inserted_function_time_period
        ), "inserted_function_time_period data_curve"
    ...
    @pytest.mark.parametrize("analyse_specific_recs", [False, True])
    @pytest.mark.parametrize("func_type", ["min", "max", "mean", "median", "area_under_curve_cf"])
    def test_all_recs_analysed_together_curve_fit_min_max_mean_median(
        self, tgui, func_type, vary_coefs, analyse_specific_recs
    ):
        """
        ...
        if func_type == "min":
            min_results = tgui.get_data_from_qtable("min", rec_from, rec_to, "curve_fitting", return_regions=True)
            assert self.check_all_sublists_are_equal(min_results), "min all regions equal"
            assert utils.allclose(np.min(data, axis=1), min_results[0], 1e-08), "min"

        elif func_type == "max":
            max_results = tgui.get_data_from_qtable("max", rec_from, rec_to, "curve_fitting", return_regions=True)
            self.check_all_sublists_are_equal(max_results), "max all regions equal"
            assert utils.allclose(np.max(data, axis=1), max_results[0], 1e-08), "b0"

        elif func_type == "mean":
            mean_results = tgui.get_data_from_qtable("mean", rec_from, rec_to, "curve_fitting", return_regions=True)
            self.check_all_sublists_are_equal(mean_results), "mean all regions equal"
            assert utils.allclose(np.mean(data, axis=1), mean_results[0], 1e-08), "mean"

        elif func_type == "median":
            median_results = tgui.get_data_from_qtable("median", rec_from, rec_to, "curve_fitting", return_regions=True)
            self.check_all_sublists_are_equal(median_results), "median"
            assert utils.allclose(np.median(data, axis=1), median_results[0], 1e-08), "median"

        elif func_type == "area_under_curve_cf":
            test_auc, test_auc_ms = self.get_area_under_curve_results_from_data(tgui, data)

            area_under_curve_results = tgui.get_data_from_qtable(
                "area_under_curve_cf",
                rec_from,
                rec_to,
                "curve_fitting",
            ...
            area_under_curve_ms_results = tgui.get_data_from_qtable(
                "area_under_curve_cf_ms",
                rec_from,
                rec_to,
                "curve_fitting",
    ...
    def get_area_under_curve_results_from_data(self, tgui, data, baseline_override=False):
        ...
        test_auc, __ = core_analysis_methods.area_under_curve_ms(
            norm_data, tgui.adata.ts
        )  # this actually works for a matrix for data, but not for ms, not intended use
    ...
    def test_varying_boundaries_across_recs(self, tgui, mode, func_type, analyse_specific_recs):
        ...
        for region_name in ["reg_1", "reg_2", "reg_3", "reg_4", "reg_5", "reg_6"]:
            for rec in range(rec_from, rec_to + 1):
                ...
                results_key = (
                    func_type if func_type in ["min", "max", "median", "mean", "area_under_curve_cf"] else "fit"
                )
                ee_data = tgui.mw.loaded_file.curve_fitting_results[region_name]["data"][rec][results_key]

                data_range = tgui.adata.im_array[rec][
                    tgui.adata.start_sample[set_rec] : tgui.adata.stop_sample[set_rec] + 1
                ]
                ...
                if func_type == "min":
                    ...
                    test_data, __ = self.get_area_under_curve_results_from_data(
                        tgui, np.atleast_2d(data_range), baseline_override=baseline
                    )

                elif func_type == "monoexp":
                    time_range = tgui.adata.time_array[rec][
                        tgui.adata.start_sample[set_rec] : tgui.adata.stop_sample[set_rec] + 1
                    ]
    ...
    def test_max_slope_curve_fitting(self, tgui):
        ...
        ) = tgui.get_axograph_max_slope_to_test_against()
        tgui.load_a_filetype("cell_5")

        tgui.set_analysis_type("curve_fitting")
        tgui.left_mouse_click(tgui.mw.mw.curve_fitting_show_dialog_button)
        tgui.switch_checkbox(tgui.mw.dialogs["curve_fitting"].dia.hide_baseline_radiobutton, on=True)
        tgui.mw.curve_fitting_regions["reg_1"].bounds["upper_exp_lr"].setBounds((0.2626, 0.2632))
        ...
        tgui.switch_checkbox(
            tgui.mw.dialogs["curve_fitting"].dia.max_slope_direction_neg_radiobutton,
            on=True,
        )
        ...
        try:
            assert (
                np.round(float(tgui.mw.mw.table_tab_tablewidget.item(2, 1).data(0)), 3) == axograph_first_event_rise_3
            )
        except:
            breakpoint()
        ...
        tgui.switch_checkbox(
            tgui.mw.dialogs["curve_fitting"].dia.max_slope_direction_pos_radiobutton,
            on=True,
        )

        tgui.mw.curve_fitting_regions["reg_1"].bounds["upper_exp_lr"].setBounds((0.2632, 0.2700))
    ...
    def test_area_under_the_curve_against_axograph(self, tgui):
        ...
        tgui.left_mouse_click(tgui.mw.mw.curve_fitting_show_dialog_button)
        tgui.switch_checkbox(tgui.mw.dialogs["curve_fitting"].dia.hide_baseline_radiobutton, on=True)
        tgui.mw.curve_fitting_regions["reg_1"].bounds["upper_exp_lr"].setBounds(
            (519 * tgui.mw.loaded_file.data.ts, 585 * tgui.mw.loaded_file.data.ts)
        )  # hard coded, taken from test_events.test_auc_against_axograph()
        tgui.mw.curve_fitting_regions["reg_1"].bounds["upper_exp_lr"].setRegion(
            (519 * tgui.mw.loaded_file.data.ts, 585 * tgui.mw.loaded_file.data.ts)
        )  # hard coded, taken from test_events.test_auc_against_axograph()
        tgui.set_combobox(tgui.mw.dialogs["curve_fitting"].dia.fit_type_combobox, 4)

        tgui.mw.cfgs.curve_fitting_region_position_configs["reg_1"]["upper_bl_lr_lowerbound"] = [
            519 * tgui.mw.loaded_file.data.ts
        ]
        tgui.left_mouse_click(tgui.mw.dialogs["curve_fitting"].dia.fit_button)
    ...
    def test_hide_baseline_and_hide_region(self, tgui, analyse_specific_recs):
        ...
        for rec in range(0, tgui.adata.num_recs):
            ...
            if analyse_specific_recs:
                if rec in np.hstack(
                    [np.arange(0, rec_from), np.arange(rec_to + 1, tgui.adata.num_recs)]
                ):  # excluded recs
    ...
    def test_differet_analysis_per_region(self, tgui):
        ...
        for region_name, func_type in zip(
            ...
            setup_and_run_curve_fitting_analysis(
                tgui,
                func_type=func_type,
        ...
        for region_name, func_type in zip(
            ...
            for rec in range(tgui.adata.num_recs):
                if func_type in ["biexp_decay", "triexp"]:
                    self.quick_override_starting_estimate_for_testing_different_curves_across_records(
                        tgui, func_type, rec
                    )
                ...
                if region_name == "reg_1":  # max
                    max_results = tgui.get_data_from_qtable(
                        "max", rec_from, rec_to, "curve_fitting", return_regions=True
                    )[0]
                    assert utils.allclose(np.max(tgui.adata.im_array[rec]), max_results[rec], 1e-08), "b0"

                if region_name == "reg_2":  # mean
                    mean_results = tgui.get_data_from_qtable(
                        "mean", rec_from, rec_to, "curve_fitting", return_regions=True
                    )[0]
                    mean_of_data = np.mean(
                        tgui.mw.loaded_file.data.im_array[rec][tgui.adata.start_sample : tgui.adata.stop_sample + 1]
                    )
                ...
                if region_name == "reg_5":  # biexp decay
                    ...
                    assert utils.allclose(
                        tau,
                        tgui.adata.get_tau("monoexp", "tau1", rec_from, rec_to, rec),
                        1e-02,
                    ), "tau1"
                ...
                if region_name == "reg_6":  # triexp
                    ...
                    assert utils.allclose(amplitude, b1_sums, 1e-1), "fail triexp"
