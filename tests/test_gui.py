...
# Test Current Record Swidgets -----------------------------------------------------------------------------------------------------------------------
    ...
    def test_current_rec_spinbox_left_button(self, tgui):
        tgui.repeat_mouse_click(tgui.mw.mw.current_rec_leftbutton,
                                tgui.num_recs * 2,
                                0.001)
    ...
    def test_current_rec_spinbox_right_button(self, tgui):
        tgui.repeat_mouse_click(tgui.mw.mw.current_rec_rightbutton,
                                tgui.num_recs * 2,
                                0.001)
        assert tgui.mw.mw.current_rec_spinbox.value() == tgui.num_recs
    ...
    def test_current_rec_slider_right_button(self, tgui):
        ...
        tgui.repeat_mouse_click(scrollbar,
                                tgui.num_recs*2,
                                0.001,
    ...
    def test_current_rec_slider_left_button(self, tgui):
        ...
        tgui.repeat_mouse_click(scrollbar,
                                tgui.num_recs * 2,
                                0.001,
                                pos=QtCore.QPoint(1, 2))
        assert tgui.mw.cfgs.main['displayed_rec'] == 0

    def test_analyse_specific_recs_from_spinbox(self, tgui):
        tgui.set_analyse_specific_recs_rec_from_spinboxes(['0'])
        for spinbox in [tgui.mw.mw.spkcnt_spike_recs_from_spinbox, tgui.mw.mw.ir_recs_from_spinbox, tgui.mw.mw.curve_fitting_recs_from_spinbox]:
    ...
    def test_analyse_specific_recs_to_spinbox(self, tgui):
        ...
        for spinbox in [tgui.mw.mw.spkcnt_spike_recs_to_spinbox, tgui.mw.mw.ir_recs_to_spinbox, tgui.mw.mw.curve_fitting_recs_to_spinbox]:
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def test_analysis_option_curve_fitting_button(self, tgui):
        tgui.mw.open_analysis_options_menu()
    ...
    def test_analysis_option_spkcnt_button(self, tgui):
        tgui.mw.open_analysis_options_menu()
        tgui.left_mouse_click(tgui.mw.dialogs['analysis_options'].dia.spike_count_button,)
    ...
    def test_file_details_dialog(self, tgui):
        ...
        assert float(fs) == round(tgui.mw.loaded_file.data.fs, 4)
    ...
    def test_copy_vm_data(self, tgui):
        ...
        assert utils.allclose(data['Vm'].to_numpy(),
                              tgui.mw.loaded_file.data.vm_array[tgui.mw.cfgs.main['displayed_rec']],
                              1e-10)

        assert utils.allclose(data['Time(s)'].to_numpy(),
    ...
    def test_copy_im_data(self, tgui):
        ...
        data = pd.read_clipboard()
        assert utils.allclose(data['Im'].to_numpy(),
